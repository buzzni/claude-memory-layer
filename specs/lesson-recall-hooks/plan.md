...
기존 완료 체크는 v2.4.0 기준이다. 후속 Desktop reliability 요청의 context-pack 계약은 query가 있을 때 unmatched fallback을 제거하고, query 생략 시 탐색 목록을 유지한다. SQLite 페이지 단위(500)와 검색 대상 전체를 구분하며 권한 선택 후 상위 3건을 유지한다. 해당 변경 및 한국어 조사 정규화·단건 조회 안내는 로컬 구현/검증했다. [context §7](./context.md#7-2026-09-20-후속-회수-수정-로컬-미배포)에 검증과 남은 범위를 기록한다. 신규 API·schema·관리 UI·배포는 포함하지 않는다.
