...
## 10. Retrieval corrections and installed artifact verification (2026-09-21)

- Kept semantic rescue OFF and frozen cosine .829 / margin .026. Narrow trust/failure clauses are removed only for eligibility checking; remaining mixed prohibitions still abstain.
- Reproduced lexical-path bypass of explicit skip/omit/ignore and Korean omission instructions; shared lesson prohibition now runs before either lexical or semantic selection. Ordinary safety constraints such as “without losing data” remain searchable.
- Reproduced exact camelCase subject lookup failure due to question boilerplate. A sole exact symbol in name/trigger can now establish relevance; missing whole-token symbols reject a candidate, preventing a similarly named symbol plus generic topic words from matching. Event/graduated policy remains unchanged.
- TDD and parent review, followed by Terra/high read-only review: 64 related tests and final full 235 files / 1,621 tests pass; typecheck, lint (45 existing warnings, no errors), build, and architecture check pass.
- Current offline calibration: lexical recall .4167, identifier .65 (previous .3167/.35); hybrid recall .90, precision 1, negative 0, identifier 1. Original held-out remains recall .6833, precision 1, negative 0, identifier 1: exit 1.
- `benchmarks/lesson-recall/release-v2-independent.json`: independent author did not read ranking/calibration/evaluation data during authoring; 20 English lessons, 60 positive queries (20 Korean/20 English/20 exact searchable identifiers), 40 unrelated/opposite negatives. Pre-evaluation SHA-256 `c9cf7ceab43ab4b5ae61c07d8312d070b7d670346c3601073dd4460a0e314b4c`. IDs/references/counts and identifier presence in name/trigger were validated before evaluation. Limitations: synthetic identifiers, one paraphrase author, Korean not independently reviewed, balanced rather than prevalence-matched negatives. Preserve this evaluated set as a regression set; do not tune labels or thresholds against it.
- Frozen final v2 measurement: hybrid recall .9833, precision .66292, negative .25, identifier 1; lexical recall .8167, precision .65333, negative .15. Both precision and negative-injection gates fail, so the release criterion remains unmet. No result was reclassified as a pass.
- Final local npm archive SHA-256 `4edb3f374c38492a84c9a23ba603c9cc224589e93da3adf13cd5bfebe398ce05` was installed with its real dependencies into an isolated prefix. Happy discovers its stable entry through real npm-root without checkout override; isolated store access and the A-approval→B-recall→ACK→exclusion integration pass. No registry publication, user install, provider call, or user database mutation.

- Final 501-lesson synthetic performance probe: cold first call 2.90ms, background warmup 3190.80ms, 30 warm queries p95 8.14ms. This is function-level, homogeneous synthetic load; not an end-to-end provider or quality gate.
